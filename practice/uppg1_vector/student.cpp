...
// 1. Skapa en konstruktor som tar x och y som parametrar (använd gärna default-värden 0).
// 2. Implementera get_x() och get_y() som returnerar värdena (glöm inte const!).
// 3. Överlagra operator+ så att man kan addera två Vector2D-objekt med varandra.

class Vector2D {
public:
    // Din konstruktor här (tips: använd medlemsinitieringslista: Vector2D(...) : x{...}, y{...} {})
    
    // Dina getters här (måste vara const-markerade)
